...
const processSearchQuery = async (query, sessionId, socket) => {
    try {
        ...
        await emitStatusUpdate(socket, sessionId, 'initializing', '🔍 Starting your search...', 800);
        ...
        if (dbResults && dbResults.length > 0) {
            ...
            source = 'database';
            searchStep = 1;
            await emitStatusUpdate(socket, sessionId, 'found', `📚 Found ${dbResults.length} relevant articles in knowledge base`, 700);
        } else {
            await emitStatusUpdate(socket, sessionId, 'analyzing', '🧠 Analyzing your query with AI semantic search...', 900);
            ...
            try {
                ...
                if (queryEmbeddings && queryEmbeddings.length > 0) {
                    ...
                    if (searchResults && searchResults.length > 0) {
                        ...
                        source = 'qdrant';
                        searchStep = 2;
                        await emitStatusUpdate(socket, sessionId, 'found', `✨ Found ${searchResults.length} semantically similar articles`, 600);
                    }
                }
            } catch (qdrantError) {
                console.error('Qdrant search error:', qdrantError);
            ...
            if (articles.length === 0) {
                await emitStatusUpdate(socket, sessionId, 'fetching', '🌐 Fetching latest news from external sources...', 900);
                ...
                try {
                    ...
                    if (freshArticles && freshArticles.length > 0) {
                        ...
                        source = 'newsapi';
                        searchStep = 3;
                        await emitStatusUpdate(socket, sessionId, 'found', `📰 Retrieved ${freshArticles.length} fresh articles`, 600);
                    }
                } catch (newsError) {
                    console.error('News API error:', newsError);
                }
        ...
        if (articles.length > 0) {
            ...
            if (validArticles.length === 0) {
                await emitStatusUpdate(socket, sessionId, 'fetching', '🔄 Searching additional sources...', 800);
                try {
                    const { fetchNews } = require('../services/newsService');
                    const freshArticles = await fetchNews(query);
                    if (freshArticles && freshArticles.length > 0) {
                        ...
                        source = 'newsapi';
                        searchStep = 3;
                        await emitStatusUpdate(socket, sessionId, 'found', `📋 Retrieved ${freshArticles.length} additional articles`, 600);
                    }
                } catch (newsError) {
                    console.error('News API fallback failed:', newsError);
                }
            ...
            await emitStatusUpdate(socket, sessionId, 'processing', '⚡ Selecting most relevant information...', 700);
            const mostRelevantArticle = selectMostRelevantArticle(articles, query);

            await emitStatusUpdate(socket, sessionId, 'generating', '🤖 AI is crafting your comprehensive response...', 1000);
...
`;
        ...
        await emitStatusUpdate(socket, sessionId, 'complete', '❌ No relevant information found', 300);
        ...
        console.error('Error in processSearchQuery:', error);
        await emitStatusUpdate(socket, sessionId, 'error', '⚠️ An error occurred during search', 300);
        ...
        socket.on('send_message', async (data) => {
            try {
                ...
                if (processingStates.has(sessionId)) {
                    console.log(`Already processing message for session ${sessionId}, ignoring duplicate`);
                    return;
                }
